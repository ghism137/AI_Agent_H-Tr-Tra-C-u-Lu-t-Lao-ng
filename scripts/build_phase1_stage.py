...
def main() -> None:
    ...
    for document in sorted(documents, key=lambda item: item["doc_id"]):
        ...
        try:
            ...
            article_numbers_raw = [int(article["article_number"]) for article in result["articles"]
                                   if article["content_kind"] == "normative" and article["article_number"].isdigit()]
            article_numbers = []
            for num in article_numbers_raw:
                if not article_numbers or article_numbers[-1] != num:
    ...
    if operations_path.exists():
        ...
        for op in operations:
            ...
            status = op.get("review_status", "")

            # Skip pending and blocked ops — they are intentionally excluded from materialization
            if status in ("pending", "blocked"):
                print(f"  Skipping op {op_id} (status={status})")
                continue

            # Verified ops MUST have real provenance hashes (not placeholders)
            if status == "verified":
                if op.get("source_hash") == "unknown":
                    gate3_blockers.append(
                        f"op {op_id}: verified but source_hash=unknown (provenance not established)"
                    )
                if op.get("target_hash") == "unknown" and op.get("verb") not in ("applicability",):
                    gate3_blockers.append(
                        f"op {op_id}: verified but target_hash=unknown for verb={op.get('verb')}"
                    )
                if not op.get("source_clause") or not op.get("effective_from") or not op.get("target_locator"):
                    gate3_blockers.append(f"op {op_id}: missing required field (source_clause/effective_from/target_locator)")

            # Reject ops with no recognized status
            elif status not in ("pending", "blocked", "verified"):
                gate3_blockers.append(f"op {op_id}: unrecognized review_status={status!r}")

        if gate3_blockers:
            for b in gate3_blockers:
                print(f"  Gate3 blocker: {b}")
            gate3 = "FAIL"

        # Gate 3 must FAIL if a mandatory relation has no corresponding operation
        relations_path = REGISTRY / "relations_verified.json"
        if relations_path.exists():
            relations = json.loads(relations_path.read_text(encoding="utf-8"))
            op_rels = {op.get("relation_id") for op in operations if op.get("relation_id")}
            for rel in relations:
                if rel.get("review_status") == "verified" and rel.get("operation") != "none":
                    if rel["relation_id"] not in op_rels:
                        print(f"  Gate3 blocker: Missing mandatory operation for relation {rel['relation_id']}")
                        gate3 = "FAIL"

    try:
        versions = materialize_versions(versions, operations)
        # Regenerate chunks using the updated versions
        from backend.ingestion.chunker_v2 import chunk_versions
        chunks = chunk_versions(versions, parsed, release_id)
    ...
    operations_sha256 = (
        hashlib.sha256(operations_path.read_bytes()).hexdigest()
        if operations_path.exists() else "absent"
    )
    manifest = {
        "release_id": release_id, "status": "candidate", "published": False,
        "sources_sha256": hashlib.sha256((REGISTRY / "sources.json").read_bytes()).hexdigest(),
        "parser_code_sha256": code_sha256,
        "verified_relations_sha256": relation_sha256,
        ...
        "missing_required_parsed_documents": missing,
        "verified_as_of": None,
