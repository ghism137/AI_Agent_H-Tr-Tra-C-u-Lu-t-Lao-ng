...
from backend.ingestion.version_builder import extract_payload, split_payload
...
for article in result['articles']:
    ...
    if 'article:1/item:5' in path:
        print(f"path: {path}")
        content = article['content']
        print(f"original content: {content[:100]}...")
        payload = extract_payload(content)
        print(f"payload length: {len(payload)}")
        subs = split_payload(payload, ['body', 'article:14', 'item:5'])
        for p, c in subs:
            print(f"  sub {p}: {len(c)} chars")
