...
document = next(d for d in json.loads((ROOT / 'data' / 'registry' / 'documents.json').read_text(encoding='utf-8')) if d['doc_id'] == 'doc:02/2025/NĐ-CP')
...
for article in result['articles']:
    if '/'.join(article['structural_path']) == 'body/article:1/item:1':
        print(f"path: {'/'.join(article['structural_path'])}")
